...
* Replacement ``url`` template tag as well as ``reverse`` and ``redirecct_to``
  ...
  (ex: http://my_app.mycompany.com/canvas/my_view to
       https://apps.facebook.com/my_app/my_view)
* Ability to define facebook test users, their permissions, and their initial
  friends per app.  Management commands ``create_facebook_test_users'` and
