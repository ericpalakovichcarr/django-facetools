...
from facetools.common import create_signed_request, _get_facetools_test_fixture_name
...
class FacebookClient(Client):
    ...
    def facebook_login(self, facebook_test_user):
        ...
        try:
            ...
            raise TestUserNotLoaded("Test user %s hasn't been loaded. Did you run sync_facebook_test_users?" % facebook_test_user)

    def facebook_logout(self):
        """
        Unsets the user for signed_requests generated by the facebook_get method.
        """
        self.test_user = None
    ...
    def facebook_get(self, path, data={}, follow=False, **extra):
        ...
        (e.g. https://apps.facebook.com/django_facetools/myview/).  Does this by
        submitting a POST instead of a GET with a signed_request in the post data.
        """
        if self.test_user:
            signed_request = create_signed_request(
                settings.FACEBOOK_APPLICATION_SECRET_KEY,
                user_id=str(self.test_user.facebook_id),
                issued_at=1254459601, # year 2009
                expires=3254459601,   # year 2073
                oauth_token=self.test_user.access_token
            )
            return self.post(path, data={'signed_request': signed_request}, follow=follow, **extra)
